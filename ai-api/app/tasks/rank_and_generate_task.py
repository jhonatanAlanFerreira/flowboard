...
from app.services.data_question.data_question_scoring_service import (
    DataQuestionScoringService,
)
...
def rank_and_generate_task(user_prompt: str, job_id: str, chunks_raw: List[dict]):
    with tracer.start_as_current_span("rank_and_generate_task") as span:
        ...
        span.set_attribute(
            "input.first_20_chunks",
            json.dumps([c.model_dump(mode="json") for c in chunks[:20]]),
        )

        top_candidates: List[ScoredChunk] = (
            data_question_scoring_service.score_and_rank_chunks(user_prompt, chunks)
        )

        re_ranked_chunks: List[ScoredChunk] = data_question_reranker_agent.rerank_tasks(
            user_prompt, top_candidates
        )

        result: DataQuestionResponse = (
            data_question_generator_agent.generate_final_answer(
                user_prompt, re_ranked_chunks[:5]
            )
        )
