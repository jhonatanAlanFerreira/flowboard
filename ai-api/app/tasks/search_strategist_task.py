...
from app.services.data_question.data_question_retrieval_service import (
    DataQuestionRetrievalService,
)
import json
...
def search_strategist_task(prompt: str, user_id: int, ai_job_id: int):
    with tracer.start_as_current_span("search_strategist") as span:
        ...
        try:
            user_workspaces = workspace_service.get_workspace_candidates(
                user_id, prompt
            )
            ...
            prediction_result = workspace_predictor_agent.predict_workspace_intent(
                prompt, user_workspaces
            )

            span.set_attribute("agent.output_prediction", json.dumps(prediction_result))

            chunks = data_question_retrieval_service.retrieve_chunks_for_question(
                prompt, user_id, prediction_result
            )
