...
from app.services.agents.generate_workflow_workspace_agent import (
    GenerateWorkflowWorkspaceAgent,
)
...
@celery.task
def generate_workflow_workspace_task(
    ...
    with tracer.start_as_current_span("workflow_workspace_generation") as span:
        ...
        try:
            ...
            selectedWorkflowLists = workflow_refiner_agent.filter_workflow_workspaces(
                workspace_patterns.get("workflowLists")
            )

            # Generate workflow via LLM
            workspace_data = generator_agent.generate_workspace_llm(
                user_prompt, selectedWorkflowLists
            )
            source_workspace_ids: List[int] = [
                int(ws["workspace_id"]) for ws in selectedWorkflowLists
            ]
