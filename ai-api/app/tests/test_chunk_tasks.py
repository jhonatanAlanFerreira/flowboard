...
def test_generate_tags_task_success(mock_dependencies):
    ...
    deps[
        "tag_agent"
    ].generate_tags.return_value = '{"tags": ["Unit Test ", "Celery !!"]}'
    ...
    deps["span"].set_attribute.assert_any_call(
        "output.tags", ["Unit Test ", "Celery !!"]
    )
...
def test_normalize_tag():
    ...
    assert (
        normalize_tag("multiple   spaces  and-dashes") == "multiple_spaces_and_dashes"
    )
