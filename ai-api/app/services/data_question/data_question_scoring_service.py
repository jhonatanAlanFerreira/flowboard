...
class DataQuestionScoringService:
    ...
    def score_and_rank_chunks(
        self, query: str, chunks: List[HydratedChunk], limit: int = None
    ) -> List[ScoredChunk]:
        """
        ...
        with tracer.start_as_current_span("service.scoring") as span:
            ...
            for chunk_data in chunks:
                ...
                status_weight = (
                    self.config.not_done_weight
                    if not chunk.done
                    else self.config.done_weight
                )

                both_weight = (
                    self.config.double_retrieval_boost if chunk.found_in_both else 0.0
                )
                ...
                chunk.final_calculated_score = round(
                    base_score + status_weight + both_weight + recency_weight, 4
                )
            ...
            ranked_chunks = sorted(
                scored_chunks, key=lambda x: x.final_calculated_score, reverse=True
            )
