...
class TaggingService:
    ...
    def create_tag_if_not_exists(self, name: str):
        # Check if tag exists
        existing = (
            self.client.query.get(self.class_name, ["name"])
        ...
        return self.client.data_object.create(
            data_object={"name": name}, class_name=self.class_name, vector=vector
        )
    ...
    def suggest_tags_for_text(self, text: str, limit: int = 5):
        ...
        response = (
            self.client.query.get(self.class_name, ["name"])
            .with_near_vector({"vector": text_vector, "distance": 0.8})
