...
class GenerateWorkflowWorkspaceAgent:
    ...
    def generate_workspace_llm(self, user_prompt: str, selected_workflow_lists: List[Dict]) -> dict:
        with tracer.start_as_current_span("service.generate_workflow_workspace") as span:
            ...
            if selected_workflow_lists:
                ...
                avg_lists = sum(len(ws.get('lists', [])) for ws in selected_workflow_lists) // len(selected_workflow_lists)
            else:
                reference_str = "REFERENCE WORKFLOWS: None provided. Use your best judgment."
                avg_lists = 4
