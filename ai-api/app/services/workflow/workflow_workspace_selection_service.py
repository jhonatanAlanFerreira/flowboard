...
class WorkflowWorkspaceSelectionService:
    def __init__(
        ...
        self.relative_threshold = (
            ...
            else self.config.relative_threshold
        )
    ...
    def _filter_min_similarity(
        self, candidates: List[WorkspaceResult]
    ) -> List[WorkspaceResult]:
        return [c for c in candidates if c.score >= self.min_similarity]

    def _filter_relative_to_best(
        self, candidates: List[WorkspaceResult], best: float
    ) -> List[WorkspaceResult]:
