...
class CollectionPatternExtractionService:
    ...
    def extract_patterns_from_lists(
        ...
        with tracer.start_as_current_span("service.patterns.extract") as span:
            ...
            for list_obj in lists_data:
                ...
                results.append(TaskListPatterns(
                    tasklist_id=list_id,
    ...
    def _filter_chunks(self, chunks: List[TaskListChunk], min_score: float) -> List[TaskListChunk]:
    ...
    def _rank_tag_groups(self, tag_groups: List[dict], chunks: List[TaskListChunk]) -> List[dict]:
        ranked_groups = []
        for group in tag_groups:
    ...
    def _compute_tag_features(self, items: List[dict], chunks: List[TaskListChunk]) -> dict:
